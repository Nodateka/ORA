...
then
    show "curl не найден. Устанавка curl..."
    sudo apt update && sudo apt upgrade && sudo apt install -y curl
else
    show "curl установлен."
fi
echo

if ! command -v docker &> /dev/null
then
    show "Docker не найден. Устанавка Docker..."
...
echo
read -p "Введи приватный ключ своего кошелька (только для ноды ORA): " PRIV_KEY
read -p "Введи свой Alchemy WSS URL в сети Ethereum Mainnet: " MAINNET_WSS
read -p "Введи свой Alchemy HTTP URL в сети Ethereum Mainnet: " MAINNET_HTTP
read -p "Введи свой Alchemy WSS URL в сети Sepolia Ethereum: " SEPOLIA_WSS
read -p "Введи свой Alchemy HTTP URL в сети Sepolia Ethereum: " SEPOLIA_HTTP
...
echo
show "Запуск Docker контейнеров в Docker Compose (может занять 5-10 минут)..."
echo
